...
function toFormData(input: ProductInput | Partial<ProductInput>) { const form = new FormData(); Object.entries(input).forEach(([key, value]) => { if (key !== "imageFile" && value !== undefined) form.append(key, String(value)); }); if (input.imageFile) form.append("image", input.imageFile); return form; }
